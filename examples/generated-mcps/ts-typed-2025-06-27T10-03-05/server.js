...
export default function createStatelessServer({ config, }) {
    ...
    // ===== GET /api/users =====
    server.tool("getApiUsers", "GET /api/users", {
        ...
        if (args.offset !== undefined)
            queryParams.append("offset", String(args.offset));
        // Add query parameter search
        if (args.search !== undefined)
            queryParams.append("search", String(args.search));
    ...
    // ===== GET /api/products =====
    server.tool("getApiProducts", "GET /api/products", {
        ...
        if (args.category !== undefined)
            queryParams.append("category", String(args.category));
        // Add query parameter minPrice
        if (args.minPrice !== undefined)
            queryParams.append("minPrice", String(args.minPrice));
        // Add query parameter maxPrice
        if (args.maxPrice !== undefined)
            queryParams.append("maxPrice", String(args.maxPrice));
        // Add query parameter sortBy
        if (args.sortBy !== undefined)
            queryParams.append("sortBy", String(args.sortBy));
        // Add query parameter inStock
        if (args.inStock !== undefined)
            queryParams.append("inStock", String(args.inStock));
        // ===== URL CONSTRUCTION =====
        if (queryParams.toString()) {
            requestOptions.url = url + (url.includes('?') ? '&' : '?') + queryParams.toString();
        }
    ...
    // ===== GET /api/users/:id =====
    server.tool("getApiUsersById", "GET /api/users/:id", {
        id: z.number()
    }, async (args) => {
        // ===== REQUEST CONFIGURATION =====
    ...
    // ===== POST /api/users =====
    server.tool("postApiUsers", "POST /api/users", {
        body: z.object({
            name: z.string(),
            email: z.string(),
            status: z.string().optional()
    ...
    // ===== PUT /api/users/:id =====
    server.tool("putApiUsersById", "PUT /api/users/:id", {
        id: z.number(),
        body: z.object({
    ...
    // ===== DELETE /api/users/:id =====
    server.tool("deleteApiUsersById", "DELETE /api/users/:id", {
        id: z.number()
    }, async (args) => {
        // ===== REQUEST CONFIGURATION =====
        const url = `http://localhost:${config.appPort || 3000}/api/users/:id`;
