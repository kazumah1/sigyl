-- Safe Fix for GitHub App User Authentication
-- This migration safely modifies the profiles table to support both Supabase auth users and GitHub App users
...
-- 3. Modify the profiles table to support both auth types (only if columns don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'auth_type') THEN
        ALTER TABLE public.profiles ADD COLUMN auth_type TEXT DEFAULT 'supabase' CHECK (auth_type IN ('supabase', 'github_app'));
    END IF;
...
-- 5. Create a new unique constraint that allows both auth types (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'profiles_auth_unique') THEN
        ALTER TABLE public.profiles ADD CONSTRAINT profiles_auth_unique UNIQUE (auth_type, auth_user_id);
    END IF;
END $$;
...
ALTER TABLE public.workspaces ADD CONSTRAINT workspaces_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
...
-- 8. Drop existing functions if they exist (to avoid parameter name conflicts)
DROP FUNCTION IF EXISTS public.create_github_app_profile(text,text,text,text,text);
DROP FUNCTION IF EXISTS public.get_or_create_github_app_profile(text,text,text,text,text);

-- 9. Create a function to handle GitHub App user profile creation
...
EXCEPTION
  WHEN unique_violation THEN
    ...
    WHERE auth_type = 'github_app' AND auth_user_id = p_github_id;
    RETURN profile_id;
  WHEN OTHERS THEN
    RAISE;
END;
...
-- 10. Create a function to get or create GitHub App user profile
...
BEGIN
  ...
  IF profile_id IS NULL THEN
    profile_id := public.create_github_app_profile(p_github_id, p_github_username, p_email, p_full_name, p_avatar_url);
  END IF;
...
-- 11. Update RLS policies to work with both auth types
...
-- 12. Create indexes for better performance (only if they don't exist)
...
-- 15. Verify the function was created successfully
SELECT 'Function get_or_create_github_app_profile created successfully' as status; 