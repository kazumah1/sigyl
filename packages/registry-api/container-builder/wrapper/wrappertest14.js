...
console.log("[WRAPPER-STAGE3] Starting MCP Wrapper v14.0.0");
console.log("[WRAPPER-STAGE3] Stage 3: Config/Secrets Management + Package Name Extraction + API Key Validation + Basic Server Creation + JSON-RPC Support");
...
(async () => {
  ...
  async function isValidSigylApiKey(apiKey) {
    ...
    // Check if this is the master key (for deployment validation)
    ...
    try {
      const response = await fetch("https://api.sigyl.dev/api/validate-key", {
        method: "POST",
  ...
  // Handle JSON-RPC requests (for deployer compatibility)
  async function handleJsonRpcRequest(req, res) {
    ...
    try {
      ...
      if (method === 'tools/list') {
        ...
        if (!config) {
          console.log("[WRAPPER-STAGE3] ⚠️ No secrets found, using default config for tools/list");
          config = {
        ...
        console.log("[WRAPPER-STAGE3] Creating MCP server for tools/list...");
        ...
        // Create a mock request/response for tools/list
        const mockReq = {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
        ...
        // Parse the response and return JSON-RPC format
        try {
          const toolsData = JSON.parse(mockRes.body);
          console.log("[WRAPPER-STAGE3] ✅ Tools fetched successfully for JSON-RPC");
          ...
          console.log("[WRAPPER-STAGE3] ❌ Error parsing tools response:", parseError.message);
          res.json({
            jsonrpc: '2.0',
            id: id,
            result: {
  ...
  app.post("/mcp", async (req, res) => {
    ...
    try {
      ...
      const apiKey = req.headers["x-sigyl-api-key"] || req.headers["authorization"]?.replace("Bearer ", "");
  ...
  app.listen(port, () => {
    ...
    console.log("[WRAPPER-STAGE3] ✅ JSON-RPC support");
