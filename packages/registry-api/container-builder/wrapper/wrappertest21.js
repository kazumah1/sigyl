...
console.log("[WRAPPER-STAGE3] Starting MCP Wrapper v21.0.0");
console.log("[WRAPPER-STAGE3] Stage 3: Environment variable fix + improved error handling");
...
(async () => {
  ...
  // Fetch secrets from database - IMPROVED with better error handling
  async function fetchSecretsFromDatabase(packageName) {
    ...
    try {
      ...
      console.log("[WRAPPER-STAGE3] SIGYL_API_KEY present:", !!sigylApiKey);
      
      if (!sigylApiKey) {
        console.log("[WRAPPER-STAGE3] ❌ SIGYL_API_KEY not found in environment");
        console.log("[WRAPPER-STAGE3] Available environment variables:", Object.keys(process.env).filter(k => k.includes('SIGYL')));
        return null;
      }

      console.log("[WRAPPER-STAGE3] Making request to Sigyl API...");
  ...
  app.get("/mcp", (req, res) => {
    ...
    res.json({
      ...
      environment: {
        sigylApiKeyPresent: !!process.env.SIGYL_API_KEY,
  ...
  app.listen(port, () => {
    console.log("[WRAPPER-STAGE3] 🚀 Improved wrapper listening on port", port);
    console.log("[WRAPPER-STAGE3] Features:");
    console.log("[WRAPPER-STAGE3] ✅ API key validation");
    console.log("[WRAPPER-STAGE3] ✅ Multi-key secrets injection");
    console.log("[WRAPPER-STAGE3] ✅ Pass-through to MCP server");
    console.log("[WRAPPER-STAGE3] ✅ Query parameter support");
    console.log("[WRAPPER-STAGE3] ✅ Enhanced error handling");
    console.log("[WRAPPER-STAGE3] ✅ Environment variable debugging");
  });
})(); 