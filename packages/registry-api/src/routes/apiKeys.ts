...
router.post('/validate', async (req: Request, res: Response) => {
  ...
  if (!apiKey) {
    ...
    return res.status(400).json({ valid: false, error: 'Missing API Key', isMaster: false });
  }
  ...
  try {
    ...
    if (user) {
      ...
      if (apiKey === process.env.SIGYL_MASTER_KEY) {
        return res.json({ valid: true, user_id: user.user_id, isMaster: true });
      } else {
        return res.json({ valid: true, user_id: user.user_id, isMaster: false });
      }
    } else {
      console.log(`❌ [API-KEY-VALIDATE] API key validation failed - Invalid key`);
      console.log(`🔍 [API-KEY-VALIDATE] === END VALIDATION REQUEST ===`);
      return res.json({ valid: false, error: 'Invalid API Key', isMaster: false });
    }
  } catch (error) {
    console.error('❌ [API-KEY-VALIDATE] Error validating API key:', error);
    console.log(`🔍 [API-KEY-VALIDATE] === END VALIDATION REQUEST ===`);
    return res.status(500).json({ valid: false, error: 'Failed to validate API key', isMaster: false });
  }
});
