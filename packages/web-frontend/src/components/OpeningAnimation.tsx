  ...
  if (variant === 'page') {
    return (
      <div className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-500 ${
        stage >= 3 ? 'opacity-0 pointer-events-none' : 'opacity-100 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500'
      }`}>
        <div className="text-center">
          <div className="relative mb-4">
            <div className={`w-12 h-12 border-3 border-white rounded-lg transition-all duration-500 ${
              stage >= 1 ? 'rotate-45 scale-110' : 'rotate-0 scale-100'
            }`} />
            <div className={`absolute inset-2 w-8 h-8 border-3 border-white/60 rounded-lg transition-all duration-500 delay-100 ${
              stage >= 2 ? 'rotate-90 scale-125' : 'rotate-0 scale-100'
            }`} />
          </div>
          <div className={`text-2xl font-bold tracking-wider transition-all duration-500 delay-200 ${
